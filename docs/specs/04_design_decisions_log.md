...
### ADR-35 — OOB-timeout sweep runs in-gateway on an interval (v1); Redis-backed paused registry
**Status:** ✅ Resolved 2026-08-31 (Unit 8, SSRF live-enablement) — realizes the sweep loop ADR-33 deferred
**Context:** Blind-SSRF confirmation (ADR-32/D-16) needs the durable `awaitOob` `interrupt()` to be *resumed* at the D-4 bound — the sweep resolver (`sweepOobTimeouts`) was built and unit-tested in Unit 5, but nothing was **running** it, and the gateway stubbed `oob.getCallback` to `null` (so even a real callback couldn't confirm). ADR-33 deferred "the standalone scan-runtime worker + the periodic OOB-timeout sweep loop" until the live testing burst landed. It has now landed (Unit 8 Phase 2), and the JWT/SQLi/IDOR classes confirm live; SSRF is the last class, gated only on the OOB listener being deployed **and** this resume loop existing. Building a separate worker + queue purely to host a 60-second interval would repeat the YAGNI ADR-33 warned against. **Decision:** (1) **Un-stub `oob.getCallback`** to read the shared `OobCallbackStore` (Redis) when OOB is fully enabled — SSRF confirmation now reads the correlated callback ledger, never a socket result (ADR-01). (2) **Run the sweep in the gateway process on a `setInterval`** (unref'd; the HTTP server owns the lifetime), configurable via `OOB_SWEEP_INTERVAL_MS` / `OOB_TIMEOUT_MS` (defaults 60 s / 5 min). Each tick calls the existing `sweepOobTimeouts` resolver, which resumes every scan past the bound and drops it from the registry on success. (3) **A Redis-backed paused registry** (`OobPausedStore`, a hash `scanId → interruptedAt`): the service detects the `oob_wait` interrupt when a graph run pauses (via the invoke result's `__interrupt__`) and calls an injected `onOobWait` port that records the scan; the sweep lists the hash to find due waits. Redis (not in-process) so a gateway restart mid-wait doesn't strand a paused scan. (4) **All-or-nothing enablement:** SSRF is only *sent* (the burst gets OOB coordinates) when the listener config **and** a shared Redis are both present — otherwise SSRF hypotheses are skipped and no scan can reach a wait it can't resolve. `resumeOobWait` is **idempotent** (it no-ops when the thread is no longer at `awaitOob`), so a duplicate tick never re-drives a terminal scan. **Consequence:** SSRF now confirms end-to-end once the listener is deployed; blind-SSRF latency ≈ the D-4 bound + up to one sweep interval (ADR-32 already accepted ~5 min). The paused registry is a **liveness** aid, not a safety gate — the durable authority for what's paused is the LangGraph checkpointer; if a registry entry were lost (e.g. a crash between the interrupt and the `onOobWait` write), that one scan could hang. **Reconstructing the registry from the checkpointer** (so a lost entry self-heals) and **moving the sweep into the standalone scan-runtime worker** (cross-replica lock for multi-instance) remain the deferred hardening ADR-33 named — promoted when the gateway runs multi-instance or a real hang is observed. **Alternatives considered:** a standalone worker + BullMQ repeatable job now (rejected for v1 — same YAGNI as ADR-33; the in-gateway interval is a composition-root detail behind the same `sweepOobTimeouts` resolver, so promoting it later is no call-site churn); listener-pushed per-token resume (already rejected in ADR-32 — multi-token re-interrupt fragility); introspecting the Postgres checkpointer for paused threads instead of a registry (deferred — brittle + LangGraph-version-specific; the Redis registry is the pragmatic v1).
**Doc:** `02` §3.2, §4.4, §9; `03` Unit 8; ADR-27, ADR-32, ADR-33

---

### ADR-36 — OOB callback uses a path token on a single host (`<base>/<token>`), not a wildcard subdomain
**Status:** ✅ Resolved 2026-09-04 (Unit 8, SSRF deploy-enablement) — refines ADR-09's listener scheme
**Context:** ADR-09/D-16 specified an Interactsh-style listener owning a **wildcard domain**, where each blind-SSRF token was a DNS label: the payload was `http://<token>.<oob-host>/` and the listener classified callback-vs-control by the request **Host**. That works but makes deploying the listener the hard part of Unit 8's last class: it requires a **domain you control plus wildcard DNS** (`*.oob.<domain>`) — no free tunnel (ngrok free, a Render/Fly app URL) gives a routable wildcard subdomain, and a paid domain + DNS propagation is exactly the kind of external lead time that blocks "confirm SSRF today." The token being a DNS label bought nothing the correlation guard doesn't already provide — the token is an unguessable 128-bit value; whether it rides in the host or the path, an unregistered token records nothing. **Decision:** Move the token into the **URL path** on a **single public host**: the payload is `<base>/<token>` (e.g. `https://abc123.ngrok-free.app/<token>`), and the listener classifies by the **first path segment** — a token-shaped segment is a callback (record + constant 200), everything else (`/register`, `/callbacks/:token`, any non-token path) falls through to the bearer-gated control routes or 404s. Host is no longer part of the decision (a tunnel/proxy rewrites it anyway; correlation, not host, is the guard). `register` now returns `{ token, base }` (the full public base URL, scheme+host) instead of `{ token, host }`; a new listener env `OOB_PUBLIC_BASE` sets that base explicitly (so TLS-fronted deploys embed `https://…`), defaulting to `http://<OOB_HOST>`. **Consequence:** the listener deploys behind **any single-host public entrypoint — a free ngrok tunnel to a local Docker container, a Render/Fly free app, or a plain box — with no wildcard DNS and no domain required.** The gateway's egress allow-list still authorizes the one OOB host (unchanged); the verify gate is unchanged (it correlates by token, never by host). Safety is preserved: control plane stays bearer-authenticated, callbacks stay correlation-gated with a constant benign 200 (no oracle), and confirmation still reads only the recorded out-of-band callback (ADR-01), never a socket result. **Alternatives considered:** keep the wildcard-subdomain scheme (rejected — the domain+wildcard-DNS requirement is the single biggest external blocker to shipping the class, for no security gain); DNS-only capture (deferred additive in ADR-32/Unit 5 — needs a DNS server, more infra, not less); a hybrid that accepts both host- and path-tokens (rejected as needless surface — one scheme is simpler and the payload construction is fully under our control).
**Doc:** `00` §(OOB listener); `02` §3.2, §4.4; `03` Unit 5/Unit 8; ADR-09, ADR-32, ADR-35; `apps/oob-listener/DEPLOY.md`
