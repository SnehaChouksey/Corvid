...
| D-12 | LLM daily spend ceilings: global + per-user hard-stop values (mechanism confirmed, ADR-21) | `02` §3.2, §7, §8 | 3 | **Resolved (values) 2026-09-02 — mechanism + defaults BUILT (Unit 3), MEASURED (Unit 8)**: real per-scan LLM cost ≈ **0.0023 credits** (hypothesize `gemini-2.5-flash` ~0.0021 + report `gemini-2.5-flash-lite` ~0.0002) from the `llm_calls` ledger → the conservative defaults (**global 5 / user 1 credits/day**) give ~430 scans/user/day headroom while bounding a runaway; **kept, now evidence-backed**. BYOK caveat: `usage.cost≈0` under BYOK, so the credit cap is inactive there — accepted for v1 |
| D-13 | JWT confusion verification signal (auth-state oracle) | `00` §9; `02` §4.4 | 5 | **Method resolved 2026-08-10; gate BUILT Unit 5** (`verifyJwt`, ADR-D13); **CALIBRATED 2026-09-02** — exact-match oracle (status+bodyHash), no numeric knob; clean separation on the testbed (alg:none == valid, ≠ no-token) across 3 runs |
| D-14 | Injection verification signal (error / dose-response time-based / boolean-diff) | `00` §9; `02` §4.4 | 5 | **Method resolved 2026-08-10; gate BUILT Unit 5** (`verifyInjection` error + dose-response, ADR-D14); **CALIBRATED 2026-09-02 — thresholds validated with margin** (MIN 1500/3500 ms, SCALE 1.6–2.4): true-positive d2≈7–8 s/d4≈15–16 s/scale 2.00–2.15, true-neg ≈0, over 3 live runs; no change needed. Boolean-differential deferred (additive) |
| D-15 | IDOR verification signal (labeled cross-session ownership proof) | `00` §9; `02` §4.4 | 5 | **Method resolved 2026-08-10; gate BUILT Unit 5** (`verifyIdor` with self/absent controls, ADR-D15); **CALIBRATED 2026-09-02** — exact-match oracle, no numeric knob; clean separation (low-priv == owner, ≠ own, absent 404, no-auth 401) across 3 runs |
| D-16 | SSRF verification signal (blind = OOB/ADR-09; non-blind canary) | `00` §9; `02` §3.2, §4.4 | 5 | **Method resolved 2026-08-10; gate BUILT Unit 5** (`verifySsrf` correlated-OOB, ADR-D16/ADR-32); non-blind canary deferred (additive); **structural (callback correlation + 10-min window), not numeric — calibration pending the OOB-listener deploy** |
